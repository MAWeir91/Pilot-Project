...
const FILES: string[] = [];
type DecideMock = Mock<[ManagerContext], Promise<unknown>>;
...
describe("manager mode", () => {
  ...
  it("does not count blocked quota-wait time against active runtime", async () => {
    ...
    const manager = fakeManagerWithImplementation(async () => {
      if ((manager.decide as Mock).mock.calls.length === 1) {
        throw quotaError;
      }
  ...
  it("does not launch a queued recovery worker before persisting the launch transition", async () => {
    ...
    const preparedJobs = jobs as typeof jobs & { prepareBuild: any; launchPreparedBuild: any };
  ...
  it("prevents duplicate worker starts while a tick is active", async () => {
    ...
    const decide: DecideMock = vi.fn(
      async (_context: ManagerContext) =>
        await new Promise<unknown>((resolve) => {
          resolveDecision = resolve;
        })
    );
    const manager: ManagerAdapter = {
      decide
    };
    const { service } = await harness({ manager, configured: true });
    const run = await createStartedRun(service);
  ...
  it("review pass reconciliation leaves the reviewing phase without asking the manager", async () => {
    ...
    jobs.reconcileTaskForAutopilot = vi.fn(async (id: string) => {
...
async function createStartedRun(service: AutopilotService, limits = {}) {
...
function fakeManager(decisions: unknown[]): ManagerAdapter & { decide: DecideMock } {
  const decide: DecideMock = vi.fn(
    async (_context: ManagerContext): Promise<unknown> => decisions.shift() ?? decision("stop", "Stop")
  );
  return { decide };
}

function fakeManagerWithImplementation(implementation: (context: ManagerContext) => Promise<unknown>): ManagerAdapter & { decide: DecideMock } {
  const decide: DecideMock = vi.fn(implementation);
  return { decide };
}

function decision(action: string, summary: string, overrides: Record<string, unknown> = {}) {
  return {
    action,
    summary,
    reason: null,
...
function fakeJobs(store: StateStore) {
  return {
    ...
    runReview: vi.fn(async (taskId: string) => {
    ...
    finalizeTask: vi.fn(async (taskId: string) => {
      await store.updateTask(taskId, (task) => ({
        ...task,
        status: "completed",
        completedAt: "2026-06-24T06:03:00.000Z"
    ...
    getTaskDetails: vi.fn(async (taskId: string) => {
    ...
    getPlanDetails: vi.fn(async (planId: string) => {
      throw new Error(`Unknown planId: ${planId}`);
    }),
    cancelDuplicateQueuedTasks: vi.fn(async (projectId?: string) => {
      const tasks = await store.listTasks();
      const cancelled: TaskRecord[] = [];
      for (const task of tasks) {
        if (projectId && task.projectId !== projectId) continue;
        if (task.status !== "queued" || task.build.status !== "queued") continue;
        ...
        cancelled.push(
          await store.updateTask(task.id, (existing) => ({
            ...existing,
            status: "stopped",
            build: {
              ...existing.build,
    ...
    cancelDuplicateQueuedTask: vi.fn(async (taskId: string, reason = "Duplicate queued task.") => {
      const tasks = await store.listTasks();
      const task = tasks.find((candidate) => candidate.id === taskId);
      if (!task) throw new Error(`Unknown taskId: ${taskId}`);
      const duplicate = tasks.find(
        (candidate) => candidate.id !== task.id && candidate.projectId === task.projectId && candidate.title === task.title
      );
      if (!duplicate) throw new Error(`Task ${taskId} does not duplicate another task.`);
    ...
    reconcileTaskForAutopilot: undefined as undefined | Mock<[string], Promise<TaskRecord>>
  };
}
...
function taskSpec(title: string) {
  return {
    title,
...
function needsFixesTask(taskId: string): TaskRecord {
  return {
    id: taskId,
    projectId: "trade-journal-lite",
    title: "Needs fixes",
...
function blockedLostWorkerTask(taskId: string): TaskRecord {
  return {
    id: taskId,
    projectId: "trade-journal-lite",
...
function recoveryQueueItem(originalTaskId: string) {
  return {
...
function preparedRecoveryTask(taskId: string): TaskRecord {
  return {
    id: taskId,
    projectId: "trade-journal-lite",
    title: "Revalidate storage",
...
function readyTask(taskId: string): TaskRecord {
  return {
    id: taskId,
    projectId: "trade-journal-lite",
    title: "Set up local Vite + TypeScript foundation",
