...
describe("local launcher core", () => {
  ...
  it("rejects malformed launcher config", () => {
    ...
    expect(() => parseLocalLauncherConfig({ PROJECT_PILOT_TUNNEL_ARGS: "run --profile" }, "C:\\project-pilot")).toThrow(
      /PROJECT_PILOT_TUNNEL_ARGS/
    );
  });
  ...
  it("finds only the configured tunnel profile process", () => {
    ...
    expect(
      findTunnelProfileProcesses(processes, {
        tunnelCommand: "tunnel-client",
        tunnelProfile: "project-pilot"
      }).map((item) => item.pid)
    ).toEqual([10]);
...
function runWithStatus(status: Extract<AutopilotStatus, "paused" | "blocked" | "completed" | "running">): AutopilotRunRecord {
  const phase: AutopilotPhase = status === "running" ? "building" : status === "completed" ? "completed" : "paused";
  return {
    id: `run-${status}`,
    projectId: "project",
    briefId: "brief",
