...
describe("job lifecycle reconciliation", () => {
  ...
  it("persists structured passed command results when a build worker completes", async () => {
    ...
    let onClose: ((exitCode: number | null, signal: NodeJS.Signals | null, stdoutText: string) => void) | undefined;
    const spawnJob = vi.fn((options: { onClose?: typeof onClose }) => {
    ...
    await fs.writeFile(
      taskBuildReportFile(projectPath, started.taskId),
      buildReportFor(started.taskId, projectPath, taskBuildReportFile(projectPath, started.taskId)),
      "utf8"
    );
    onClose?.(0, null, "");
    await waitFor(async () => {
  ...
  it("reconciles a completed review worker from reports after restart", async () => {
    ...
    await fs.writeFile(
      taskBuildReportFile(projectPath, TASK_ID),
      buildReportFor(TASK_ID, projectPath, taskBuildReportFile(projectPath, TASK_ID)),
      "utf8"
    );
    await fs.writeFile(
      taskReviewReportFile(projectPath, TASK_ID),
      reviewReportFor(TASK_ID, projectPath, taskReviewReportFile(projectPath, TASK_ID)),
      "utf8"
    );
    const projects = new ProjectRegistry(registryFile, () => "2026-06-24T02:10:00.000Z");
    await projects.registerProject({
...
function buildReportFor(taskId: string, executionRoot: string, reportPath: string): string {
  return [
    "# Build Report",
    "",
    "Report Type: build",
...
function reviewReportFor(taskId: string, executionRoot: string, reportPath: string): string {
  return [
    "# Review Report",
    "",
    "Report Type: review",
