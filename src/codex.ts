...
export type SpawnedCodexJob = ChildProcessWithoutNullStreams;
...
export function assertPlanId(planId: string): string {
...
export function buildCodexExecArgs(projectRoot: string, sandbox: CodexSandbox = "danger-full-access"): string[] {
...
export function buildLogPath(taskId: string, kind: "build" | "review"): string {
  assertTaskId(taskId);
  return dataPath(`${taskId}.${kind}.jsonl`);
}

export function buildPlanLogPath(planId: string): string {
  assertPlanId(planId);
...
export function spawnCodexJob(options: {
  ...
  prompt: string;
  logPath: string;
  onExit: (exitCode: number | null, signal: NodeJS.Signals | null, stdoutText: string) => void;
  ...
  child.on("error", (error) => {
    log.write(
      `${JSON.stringify({ type: "error", timestamp: new Date().toISOString(), text: error.message })}\n`
    );
  ...
  child.on("exit", (exitCode, signal) => {
    log.write(
      `${JSON.stringify({ type: "exit", timestamp: new Date().toISOString(), exitCode, signal })}\n`
    );
  ...
  child.on("close", (exitCode, signal) => {
    log.write(
      `${JSON.stringify({ type: "close", timestamp: new Date().toISOString(), exitCode, signal })}\n`
    );
    log.end();
