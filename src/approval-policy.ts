...
const DEFAULT_APPROVAL_MODE: ApprovalMode = "auto_for_safe_tasks";

const RISK_PATTERNS: Array<{ flag: RiskFlag; patterns: RegExp[] }> = [
  {
    flag: "deployment",
    patterns: [
...
export function projectVerificationCommands(project: ProjectRecord): string[] {
...
function detectRiskEvidence(task: TaskRecord, buildReport?: string | null, reviewReport?: string | null): RiskEvidence[] {
          ...
          if (hasNegatedContext(source.text, index) || isEvidenceOfAbsence(source.text, index)) {
            evidence.push(
              unsupportedEvidence(
                ...
                source,
                match[0],
                "Risk wording appears only in negative/exclusion context.",
                index
              )
            );
            continue;
...
function uniqueRiskFlags(evidence: RiskEvidence[]): RiskFlag[] {
  return [...new Set(evidence.filter((item) => item.confidence !== "unsupported").map((item) => item.flag))].sort();
}

function riskSources(task: TaskRecord, buildReport?: string | null, reviewReport?: string | null): Array<{
...
function detectCredentialEvidence(source: { source: RiskEvidenceSource; sourcePath: string; text: string }): RiskEvidence[] {
  const evidence: RiskEvidence[] = [];
  const rules: Array<{ pattern: RegExp; behavior: (match: RegExpMatchArray) => string; rule: string }> = [
    {
      ...
      if (hasNegatedContext(source.text, index) || isEvidenceOfAbsence(source.text, index)) {
        evidence.push(
          unsupportedEvidence(
            ...
            source,
            rule.behavior(match),
            "Credential/secret wording appears only in negative/exclusion context.",
            index
          )
        );
        continue;
...
function unsupportedEvidence(
  flag: RiskFlag,
  source: { source: RiskEvidenceSource; sourcePath: string; text: string },
...
function reportOnlyPolicyRule(flag: RiskFlag): string {
...
function policyRuleFor(flag: RiskFlag): string {
  switch (flag) {
    case "deployment":
...
function dedupeRiskEvidence(evidence: RiskEvidence[]): RiskEvidence[] {
...
function hasNegatedContext(text: string, index: number): boolean {
...
function isEvidenceOfAbsence(text: string, index: number): boolean {
  const sentence = sentenceAround(text, index).toLowerCase();
...
function hasImplementationFileEvidence(text: string): boolean {
  return (
    /\b(?:src|app|lib|server|client|public|config|scripts|\.github)[\\/\w.-]*\.(?:ts|tsx|js|jsx|json|ya?ml|toml|md|env|ini|sh|ps1)\b/i.test(
      text
    ) ||
...
function hasConcreteBehaviorFor(flag: RiskFlag, text: string): boolean {
  const normalized = text.toLowerCase();
  switch (flag) {
    case "deployment":
      return /\b(deploy script|deployment workflow|release command|production environment|hosting provider|vercel --prod|netlify deploy|firebase deploy|wrangler deploy|gh-pages|npm run deploy)\b/i.test(
        normalized
      );
    case "payments_or_spending":
      return /\b(stripe|paypal|square|braintree|adyen|checkout session|payment intent|billing api|payment sdk|subscription flow|purchase flow|transaction endpoint|charge money)\b/i.test(
        normalized
      );
    case "credentials_or_secrets":
      return /\b(\.env|api[_ -]?key|secret|token|password|credential|oauth|authorization header|bearer token|process\.env|import\.meta\.env)\b/i.test(
        normalized
      );
    case "external_service_integration":
      return /\b(remote api|external api|external service|third-party service|webhook|cloud service|https?:\/\/|fetch|axios)\b/i.test(
        normalized
      );
    case "brokerage_or_trading":
      return /\b(brokerage api|financial api|trading account|real trading|live trading|place order|submit order)\b/i.test(
        normalized
      );
    case "dangerous_git_operation":
      return /\b(force push|--force|delete branch|git reset --hard|rewrite git history|branch protection|protected branch|organization settings|repository permissions|push directly to main|merge pull request)\b/i.test(
        normalized
      );
    case "production_database_migration":
      return /\b(production database migration|migrate production database)\b/i.test(normalized);
    case "data_deletion":
      return /\b(delete user data|delete project data|drop user data|drop project data|destructive data)\b/i.test(
        normalized
      );
    case "network_exposure":
...
function extractRiskRelevantReportText(report?: string | null): string {
  if (!report) {
    return "";
  }
  return report
