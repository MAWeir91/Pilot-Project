...
import type {
  AutopilotRunLimits,
  AutopilotRunRecord,
  AutopilotRuntimeSnapshot,
  AutopilotStatus,
  AutopilotWorkerLease,
  AutopilotWorkerLeaseStatus,
  AutopilotTaskQueueItem,
  AutopilotAttemptType,
...
import type { JobService, PreparedBuild } from "./jobs.js";
...
export type ManagerDecision = z.infer<typeof decisionSchema>;
...
export interface ManagerContext {
  ...
  projectTasks: Array<
    Pick<TaskSummary, "taskId" | "title" | "status" | "updatedAt" | "buildSummary" | "reviewResult" | "approval">
  >;
  reports: {
...
export interface ManagerAdapter {
  decide(context: ManagerContext): Promise<unknown>;
}

interface ManagerCandidateDiagnostics {
  source: string;
  selectedOutputItemPreview: string | null;
  candidatePreview: string;
  receivedAction: string | null;
  missingFields: string[];
}

interface ManagerCandidateEnvelope {
  candidate: unknown;
  diagnostics: ManagerCandidateDiagnostics;
}
...
export interface AutopilotNotifier {
  notify(title: string, message: string): void;
}

export class NullAutopilotNotifier implements AutopilotNotifier {
  notify(): void {
...
export interface ManagerModeConfig {
  configured: boolean;
  managerModel: string;
  maxManagerDecisionsPerRun: number;
  maxTasksPerRun: number;
  maxFixAttemptsPerTask: number;
  maxManagerRuntimeMs: number;
}

export function managerModeConfig(env = process.env): ManagerModeConfig {
  return {
    ...
    maxFixAttemptsPerTask: positiveInt(
      env.PROJECT_PILOT_MANAGER_MAX_FIX_ATTEMPTS_PER_TASK,
...
export function managerConfigurationStatus(config = managerModeConfig()): Record<string, unknown> {
  return {
...
export function validateManagerDecision(value: unknown): ManagerDecision {
  return decisionSchema.parse(value);
}

export class OpenAIResponsesManagerAdapter implements ManagerAdapter {
  constructor(private readonly config = managerModeConfig()) {}

  async decide(context: ManagerContext): Promise<unknown> {
...
export class CodexArchitectAdapter implements ArchitectAdapter {
  async consult(options: { projectRoot: string; prompt: string; threadId?: string }): Promise<{ threadId?: string; summary: string }> {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey?.trim()) {
      throw new Error("OPENAI_API_KEY is not configured.");
    }
...
export class AutopilotService {
  ...
  private readonly architect: ArchitectAdapter;
  private readonly notifier: AutopilotNotifier;
  private readonly config: ManagerModeConfig;
  ...
  constructor(options: AutopilotServiceOptions) {
  ...
  async configurationStatus(): Promise<Record<string, unknown>> {
  ...
  async createProjectBrief(input: ProjectBriefInput): Promise<{ briefId: string; brief: ProjectBriefRecord }> {
  ...
  async getProjectBrief(briefId: string): Promise<ProjectBriefRecord> {
  ...
  async listProjectBriefs(): Promise<{ briefs: ProjectBriefRecord[] }> {
  ...
  async startAutopilot(input: {
    ...
    briefId: string;
    planId?: string;
    limits?: Partial<AutopilotRunLimits>;
  }): Promise<{ runId: string; status: AutopilotStatus; run: AutopilotRunRecord }> {
  ...
  async updateAutopilotLimits(input: {
    ...
    maxTasks?: number;
    maxFixAttemptsPerTask?: number;
  }): Promise<AutopilotRunRecord & { runtime: AutopilotRuntimeSnapshot }> {
    const now = this.now();
    const run = await this.requireRun(input.runId);
    const recalculatedActiveRuntimeMs =
      run.activeRuntimeMs === undefined ? await this.recalculateActiveRuntimeMs(run) : runtimeSnapshot(run, now).activeRuntimeMs;
    const updated = await this.updateRun(input.runId, (existing) => {
      ...
      const wasRuntimeLimited =
        existing.status === "usage-limited" && /runtime budget|runtime limit/i.test(existing.pauseReason ?? "");
      return {
        ...existing,
        status: wasRuntimeLimited && remaining > 0 ? "paused" : existing.status,
        phase: wasRuntimeLimited && remaining > 0 ? "paused" : existing.phase,
        pauseReason:
          wasRuntimeLimited && remaining > 0
  ...
  async stopAutopilot(runId: string, reason = "Stopped by user request."): Promise<AutopilotRunRecord> {
    const now = this.now();
    const run = await this.updateRun(runId, (existing) => ({
  ...
  async reconcileAndResume(): Promise<void> {
    const runs = await this.store.listAutopilotRuns();
        ...
        if (run.activeRuntimeStartedAt) {
          await this.updateRun(run.id, (existing) => ({
            ...
            timeline: appendTimeline(
              ...
              this.now(),
              "status",
              "Runtime tracking resumed after Project Pilot startup; server-down time was excluded."
            )
          }));
        }
  ...
  private scheduleRun(runId: string, reason = "scheduled"): void {
  ...
  private async handleSchedulerError(runId: string, error: unknown): Promise<void> {
    const message = errorMessage(error);
    await this.updateRun(runId, (existing) =>
      pauseRun(existing, `state_store_unavailable: Scheduler could not safely persist state. ${message}`, this.now(), "blocked")
    ).catch(() => undefined);
    this.notifier.notify("Project Pilot state store unavailable", message);
  }

  private async tickInternal(runId: string): Promise<void> {
    ...
    if (cancelledDuplicates.length > 0) {
      await this.updateRun(run.id, (existing) => ({
        ...
        timeline: appendTimeline(
          ...
          this.now(),
          "status",
          `Cancelled ${cancelledDuplicates.length} duplicate queued task(s) before manager decision.`,
          { taskIds: cancelledDuplicates.map((task) => task.id) }
        )
      }));
      run = await this.requireRun(run.id);
    }
  ...
  private async requestManagerDecision(
    run: AutopilotRunRecord,
    brief: ProjectBriefRecord,
    project: ProjectRecord
    ...
    await this.updateRun(run.id, (existing) => ({
      ...
      timeline: appendTimeline(
        ...
        this.now(),
        "status",
        `manager_decision_invalid: malformed manager decision; requesting one corrective retry. ${firstDiagnostics}`,
        {
    ...
    await this.pauseFor(
      run.id,
      `manager_decision_invalid: manager returned malformed decisions twice. First validation: ${firstDiagnostics}. Second validation: ${secondDiagnostics}.`,
      "blocked"
    );
    return null;
  }

  private async recordCandidateDiagnostics(runId: string, diagnostics: ManagerCandidateDiagnostics): Promise<void> {
    ...
    await this.updateRun(runId, (existing) => ({
      ...
      timeline: appendTimeline(
        ...
        this.now(),
        "status",
        `manager_decision_candidate: action=${action}; missing=${missing}.`,
        { ...diagnostics }
      )
    }));
  }

  private async buildManagerContext(
    run: AutopilotRunRecord,
    brief: ProjectBriefRecord,
    project: ProjectRecord
  ...
  private async advanceActiveTask(run: AutopilotRunRecord, task: TaskRecord): Promise<boolean> {
    const reconciled =
      typeof (this.jobs as unknown as { reconcileTaskForAutopilot?: unknown }).reconcileTaskForAutopilot === "function"
        ? await (this.jobs as unknown as { reconcileTaskForAutopilot: (taskId: string) => Promise<TaskRecord> }).reconcileTaskForAutopilot(
            task.id
          )
        : task;
    ...
    if (status === "ready-for-approval") {
      ...
      const completed = (finalized as { status?: unknown }).status === "completed";
      await this.updateRun(run.id, (existing) => ({
        ...existing,
        ...
        timeline: appendTimeline(
          ...
          this.now(),
          "reviewer-summary",
          completed ? `Task ${reconciled.id} reviewed and finalized.` : `Task ${reconciled.id} requires manual approval.`
        )
      }));
      if (!completed) {
    ...
    switch (decision.action) {
      ...
      case "create_ordered_tasks": {
        ...
        await this.updateRun(runId, (existing) => ({
          ...
          queue: [
            ...existing.queue,
            ...tasks.map((task): AutopilotTaskQueueItem => ({
  ...
  private async finalizeCurrentTask(runId: string, taskId: string): Promise<void> {
    ...
    await this.updateRun(runId, (existing) => ({
      ...
      timeline: appendTimeline(
        ...
        this.now(),
        "reviewer-summary",
        completed ? `Task ${taskId} finalized by manager decision.` : `Task ${taskId} could not be auto-finalized by policy.`
      )
    }));
    if (!completed) {
  ...
  private async skipDuplicateTask(runId: string, projectId: string, taskId: string, reason: string): Promise<void> {
    ...
    await this.updateRun(runId, (existing) => ({
      ...
      timeline: appendTimeline(
        ...
        this.now(),
        "status",
        `Skipped duplicate task ${cancelled.id}: ${reason}`,
        { taskId, projectId, reason }
      )
    }));
  }

  private async ensureRecoveryQueued(run: AutopilotRunRecord, task: TaskRecord): Promise<boolean> {
    ...
    const existing = run.queue.find(
      (item) => item.fixAttemptForTaskId === task.id && (item.source === "recovery" || item.source === "fix") && item.status === "queued"
    );
    await this.updateRun(run.id, (current) => ({
      ...
      queue: existing
        ? current.queue.map((item) =>
            item.id === existing.id ? { ...item, source: "recovery", updatedAt: this.now() } : item
          )
        : [
            ...current.queue,
            {
      ...
      timeline: appendTimeline(
        ...
        this.now(),
        "status",
        `Operational recovery queued for ${task.id} after missing terminal worker result.`,
        { taskId: task.id, recoveryAttempts: attempts }
      )
    }));
    if (this.autoSchedule) {
  ...
  private async dispatchRunnableQueueItem(runId: string, projectId: string): Promise<boolean> {
    const run = await this.requireRun(runId);
    ...
    const next =
      run.queue.find((item) => item.status === "queued" && (item.source === "recovery" || item.source === "fix")) ??
  ...
  private preflightProject(project: ProjectRecord): { ok: boolean; reason?: string; diagnostics: Record<string, unknown> } {
    const preflightRunner = this.jobs as JobService & {
      preflightWorkerLaunch?: (project: ProjectRecord) => { ok: boolean; reason?: string; diagnostics: Record<string, unknown> };
    };
  ...
  private async startQueuedTask(runId: string, projectId: string, next: AutopilotTaskQueueItem): Promise<void> {
    ...
    if (next.source === "recovery" && next.fixAttemptForTaskId) {
      ...
      if (attempts >= MAX_RECOVERY_ATTEMPTS_PER_TASK) {
        await this.pauseFor(
          runId,
          `Operational recovery for ${next.fixAttemptForTaskId} already reached ${attempts} attempt(s).`,
          "blocked"
        );
        return;
      }
    ...
    if (!preflight.ok) {
      await this.updateRun(runId, (existing) => ({
        ...
        timeline: appendTimeline(
          ...
          this.now(),
          "status",
          `Maintenance worker launch blocked: ${preflight.reason ?? "Git preflight failed."}`,
          { diagnostics: preflight.diagnostics }
        )
      }));
      await this.pauseFor(runId, preflight.reason ?? "Maintenance Git preflight failed.", "blocked");
    ...
    const preparedStarter = this.jobs as JobService & {
      prepareBuild?: (input: TaskInput) => Promise<PreparedBuild>;
      launchPreparedBuild?: (prepared: PreparedBuild) => Promise<void>;
    };
  ...
  private async startNextQueuedTask(runId: string, projectId: string): Promise<void> {
    const run = await this.requireRun(runId);
    if (run.currentTaskId) {
      return;
    }
  ...
  private async recordSchedulerOutcome(runId: string, status: string, reason: string): Promise<void> {
    await this.updateRun(runId, (existing) => ({
      ...existing,
      scheduler: {
        ...(existing.scheduler ?? {}),
  ...
  private async reconcileWorkerLeases(runId: string): Promise<void> {
    const run = await this.requireRun(runId);
    ...
    const reconciled: Array<{ leaseId: string; taskId: string; status: AutopilotWorkerLeaseStatus; outcome: string }> = [];
  ...
  private classifyWorkerLease(
    lease: AutopilotWorkerLease,
    task: TaskRecord | undefined,
    activeTaskIds: Set<string | undefined>
  ): { status: AutopilotWorkerLeaseStatus; outcome: string } | undefined {
  ...
  private async pauseFor(
    runId: string,
    reason: string,
    status: Extract<AutopilotStatus, "paused" | "blocked" | "usage-limited">
  ): Promise<void> {
  ...
  private async requireRun(runId: string): Promise<AutopilotRunRecord> {
  ...
  private async beginActiveRuntime(runId: string): Promise<void> {
    const now = this.now();
    await this.updateRun(runId, (existing) => {
  ...
  private async endActiveRuntimeIfIdle(runId: string): Promise<void> {
    const run = await this.store.getAutopilotRun(runId);
  ...
  private async recalculateActiveRuntimeMs(run: AutopilotRunRecord): Promise<number> {
...
function pauseRun(
  ...
  reason: string,
  at: string,
  status: Extract<AutopilotStatus, "paused" | "blocked" | "usage-limited"> = "paused"
): AutopilotRunRecord {
...
function shouldKeepScheduling(run: AutopilotRunRecord): boolean {
  return run.status === "running" || run.status === "queued";
}

export function runtimeSnapshot(run: AutopilotRunRecord, at = new Date().toISOString()): AutopilotRuntimeSnapshot {
...
function accrueActiveRuntime(run: AutopilotRunRecord, at: string): AutopilotRunRecord {
  if (!run.activeRuntimeStartedAt) {
...
function shouldKeepActiveRuntimeOpen(run: AutopilotRunRecord): boolean {
  return (
...
function estimateTimelineActiveRuntime(run: AutopilotRunRecord): number {
  const events = [...run.timeline].sort((left, right) => left.at.localeCompare(right.at));
  let activeStartedAt: string | undefined = run.startedAt;
  let total = 0;
...
function isTimelineActiveStop(event: AutopilotRunRecord["timeline"][number]): boolean {
  return (
    event.kind === "manager-decision" ||
    /Manager API|manager_api_|manager_decision_invalid|Manager runtime budget reached|Paused|requires manual approval|reviewed and finalized|ended with status/i.test(
      event.summary
    )
  );
}

function intervalMs(start?: string, end?: string): number {
  if (!start || !end) {
    return 0;
  }
...
function appendTimeline(
  run: AutopilotRunRecord,
  at: string,
  kind: AutopilotRunRecord["timeline"][number]["kind"],
  summary: string,
...
function markQueueTaskCompleted(queue: AutopilotTaskQueueItem[], taskId: string, at: string): AutopilotTaskQueueItem[] {
  return queue.map((item) => (item.taskId === taskId ? { ...item, status: "completed", updatedAt: at } : item));
}

function launchQueuedTaskRunState(
  run: AutopilotRunRecord,
  queueItem: AutopilotTaskQueueItem,
  taskId: string,
  lease: AutopilotWorkerLease,
  at: string
): AutopilotRunRecord {
  return {
    ...
    recoveryAttemptsByTaskId:
      queueItem.source === "recovery" && queueItem.fixAttemptForTaskId
        ? {
    ...
    timeline: appendTimeline(
      run,
      at,
      queueItem.source === "recovery" ? "status" : "builder-summary",
      `${queueItem.source === "recovery" ? "Operational recovery" : "Started task"} ${taskId}: ${queueItem.title}`,
      { queueItemId: queueItem.id, source: queueItem.source, originalTaskId: queueItem.fixAttemptForTaskId }
    )
  };
}
...
function createWorkerLease(options: {
  runId: string;
  taskId: string;
  phase: AutopilotWorkerLease["phase"];
...
function markWorkerLeases(
  leases: AutopilotWorkerLease[] | undefined,
  taskId: string,
  status: AutopilotWorkerLease["status"],
  at: string,
  outcome: string
): AutopilotWorkerLease[] {
  return (leases ?? []).map((lease) =>
    lease.taskId === taskId && lease.status === "active"
      ? { ...lease, status, endedAt: at, lastActivityAt: at, outcome }
      : lease
  );
}

function isOperationalRecoveryDecision(decision: ManagerDecision): boolean {
...
function isOperationalWorkerLoss(task: TaskRecord): boolean {
...
function managerPrompt(context: ManagerContext): string {
  return [
...
function normalizeManagerCandidate(value: unknown): ManagerCandidateEnvelope {
...
function isManagerCandidateEnvelope(value: unknown): value is ManagerCandidateEnvelope {
  return (
    typeof value === "object" &&
    value !== null &&
    "candidate" in value &&
    "diagnostics" in value &&
    typeof (value as { diagnostics?: unknown }).diagnostics === "object"
  );
}

function extractOpenAIManagerCandidate(response: any): ManagerCandidateEnvelope {
...
function selectStructuredOutputItem(response: any): { text: string; item: unknown; refusal: boolean } | undefined {
...
function managerCandidateDiagnostics(
  source: string,
  selectedOutputItem: unknown,
  candidate: unknown
): ManagerCandidateDiagnostics {
  const record = typeof candidate === "object" && candidate !== null && !Array.isArray(candidate) ? (candidate as Record<string, unknown>) : {};
  return {
    source,
...
function parseManagerDecision(value: unknown): { ok: true; decision: ManagerDecision } | { ok: false; error: unknown } {
  try {
...
function formatDecisionValidationError(error: unknown): string {
...
function managerApiFailureReason(error: unknown): string {
  const message = errorMessage(error);
...
export function projectHasRiskyRequirements(brief: ProjectBriefRecord): boolean {
  const fakeTask = {
    ...
    build: { status: "queued" as const, logPath: "unused" }
  };
