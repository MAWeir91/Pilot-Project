...
describe("planning lifecycle", () => {
    it("runs planning in read-only mode and writes PLAN_REPORT.md", async () => {
        ...
        await expect(fs.readFile(path.join(project.path, "PLAN_REPORT.md"), "utf8")).resolves.toMatch(/Plan dashboard changes|Recommended Architecture/);
    });
    ...
    it("rejects planning for unregistered projects", async () => {
        ...
        await expect(service.startPlan({
...
async function serviceWithRegisteredProject(options = {}) {
    ...
    return { service, project: project, spawnJob, store };
}
