...
describe("maintenance execution isolation", () => {
    it("persists explicit maintenance configuration and reads it back through project views", async () => {
        ...
        expect(String(status.manualHandoff.message)).toMatch(/manual/i);
    });
    ...
    it("routes Autopilot queued work to the configured execution root after valid configuration", async () => {
        ...
        expect(runs[0]?.workers?.[0]?.reportPath).toBe(path.join(executionRoot, ".project-pilot", "reports", String(runs[0]?.workers?.[0]?.taskId), "BUILD_REPORT.md"));
    });
...
async function serviceWithMaintenanceProject(options = {}) {
    ...
    return { service, registry, project: project, liveRoot, executionRoot: executionRoot ?? isolatedRoot, spawnJob, store };
}
...
function validMaintenanceGitRunner(branch, liveRoot, executionRoot, statusOutput) {
    return (args, cwd) => {
        if (args[0] === "rev-parse")
            return `${cwd}\n`;
        if (args[0] === "branch")
...
function autopilotForMaintenance(options) {
...
async function waitForPlanStatus(service, planId, status) {
...
async function expectLiveRootUnchanged(liveRoot) {
