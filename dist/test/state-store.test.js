...
afterEach(async () => {
    vi.restoreAllMocks();
    await Promise.all(FILES.map((file) => fs.rm(file, { force: true, recursive: true }).catch(() => undefined)));
    FILES.length = 0;
});
describe("durable state store", () => {
    ...
    it("retries transient EBUSY on read and write", async () => {
        ...
        const durable = new DurableJsonFile(stateFile, () => ({ tasks: [] }), normalizeTaskState, {
    ...
    it("an interrupted replacement never corrupts the prior live JSON file", async () => {
        ...
        await expect(durable.write({ tasks: [{ id: "new" }] })).rejects.toBeInstanceOf(StateStoreError);
