...
import { findTunnelProfileProcesses, hasActiveAutopilotRun, ownedStopTargets, parseLocalLauncherConfig, startPreflight } from "./local-launcher-core.js";
...
async function printStatus(config) {
    ...
    console.log(`[status] Tunnel profile ${config.tunnelProfile}: ${tunnelProcesses.length > 0 ? `running as PID(s) ${tunnelProcesses.map((item) => item.pid).join(", ")}` : "not running"}`);
    console.log(`[status] Launcher ownership: ${launcherState ? `pilot PID ${launcherState.pilotPid ?? "unknown"}, tunnel PID ${launcherState.tunnelPid ?? "unknown"}` : "none"}`);
...
async function stopOwnedProcesses(config) {
    await warnIfActiveRun();
...
function writePrefixed(prefix, chunk, error) {
...
async function dashboardHealth(config) {
...
async function validateTunnelExecutable(config) {
    const resolved = await resolveExecutable(config.tunnelCommand);
    if (!resolved) {
        throw new Error(`Tunnel executable not found: ${config.tunnelCommand}. Set PROJECT_PILOT_TUNNEL_COMMAND in .env to the tunnel-client executable path or add it to PATH.`);
    }
}
...
async function warnIfActiveRun() {
...
async function readTaskState() {
    try {
        ...
        return { exists: true, valid: true, state: JSON.parse(raw) };
    }
    catch (error) {
...
async function readLauncherState(config) {
    try {
        return JSON.parse(await fs.readFile(config.launcherStateFile, "utf8"));
    }
    catch {
        return undefined;
    }
}
async function writeLauncherState(config, state) {
    await fs.mkdir(DATA_DIR, { recursive: true });
    await fs.writeFile(config.launcherStateFile, `${JSON.stringify(state, null, 2)}\n`, "utf8");
}
async function removeLauncherState(config) {
